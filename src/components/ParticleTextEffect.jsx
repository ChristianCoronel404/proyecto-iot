...
const DEFAULT_WORDS = ["DRAKO", "TU PRÓXIMO", "GUÍA", "EN LA", "UCB"];
...
export default function ParticleTextEffect({ words = DEFAULT_WORDS }) {
  ...
  const nextWord = (word, canvas) => {
    ...
    offscreenCtx.font = "bold 130px Orbitron, Arial"; // Adaptado a la fuente tecnológica del proyecto Drako
    ...
    // Generar un color temático de Drako (Verde, Azul) en lugar de totalmente aleatorio
    const themeColors = [
      { r: 120, g: 186, b: 73 }, // Verde UCB Drako
      { r: 10, g: 67, b: 111 },  // Azul UCB
  ...
  const animate = () => {
    ...
    // Reducido a transparente para mezclarse con el backgroud radial de la landing de drako
  ...
  useEffect(() => {
    ...
    const initCanvas = () => {
      ...
      canvas.height = Math.max(500, window.innerHeight * 0.6); // Mínimo 500px o 60% de la pantalla
      nextWord(words[wordIndexRef.current], canvas);
    };
